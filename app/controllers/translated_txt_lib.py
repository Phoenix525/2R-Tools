...
def __walk_file():
    ...
    # 如果文件夹不存在，新建一个
    if not GlobalData.wait_for_entry.exists():
        print("无待录入文本，译文库无需更新！")
        GlobalData.wait_for_entry.mkdir(parents=True)
        return

    print("正在更新译文库……\n")
    global __txt_library_cache
    # 遍历所有文件
    for file in GlobalData.wait_for_entry.rglob("*"):
        if not file.is_file():
            continue

        file_name = file.name
        # 扫描renpy翻译文件
        if validate_renpy_trans_file(file):
            print(f"当前扫描文本：{file_name}")
            __scanning_rpy_file(file, __txt_library_cache, True)
            print(f"{file_name} 扫描完成！\n")
            continue

        # 扫描json文件，这里即便是扩展名非json的文件，只要内容符合标准json格式也进行处理
        json_datas = read_json(file)
        if json_datas:
            print(f"当前扫描文本：{file_name}")
...
def __scanning_rpy_file(
    file_path: str,
    filename: str,
    txt_libraries: dict[str, str] = None,
    rewrite: bool = False,
):
    """
    扫描原翻译文本，将需要的数据存入缓存器
    ...
    with open(file, "r", encoding=get_file_encoding(file)) as inp:
        for line in inp:
            # 删除末尾换行符
            line = line.rstrip("\n")
...
def __write_to_translib():
    """
    写入译文库
    """
    translated_txt_lib = deepcopy(GlobalData.translated_lib_library)
    for k, v in __txt_library_cache.items():
        # 如果缓存库查询到的键或值不是字串，为错误数据，跳过以屏蔽
        if not isinstance(k, str) or not isinstance(v, str):
            continue
        # 将含有英文的原文本统一调整为大写字母，方便统一查询。
        # 这里大部分语种都能正常处理，土耳其语等语种可能会出现奇怪的结果，但因为这些语种使用的可能性不大，这里就不做单独处理了。
        k = k.upper()
        # 译文库中已有该文本，跳过

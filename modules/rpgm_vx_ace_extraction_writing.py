...
def sacnning_common_events(json_datas: list, _type: str) -> bool:
    ...
    for json_datas_idx, json_datas_item in enumerate(json_datas):
        ...
        # list是列表
        if (
            not KEY_LIST in json_datas_item
    ...
    if KEY_TERMS in json_datas:
        ...
        if (
            ...
            for idx, etype in enumerate(_terms[KEY_ETYPES]):
                ...
                if _type == EXTRACT:
                    _change = switch_change_mark(_change, write_in_cache(etype, _loc))
                else:
                    json_datas[KEY_TERMS][KEY_ETYPES][idx] = read_from_cache(
                        etype, _loc
                    )
...
'''
    ...
    change_phoenix_mark(_game_txt_cache)
    # 更新翻译项目
    write_json(curr_rpgm_project_abspath, _game_txt_cache, indent=2)
